...
import { deletePhraseAction } from '../offline/actions';
...
export function PhraseDeleteButton({ id, label }: { id: string; label: string }) {
  ...
  return (
    <button
      ...
      disabled={busy}
      title={`Delete “${label}”`}
      aria-label={`Delete phrase ${id}`}
      className={`${iconBtn} bg-white text-red-600 hover:bg-red-50`}
      onClick={async () => {
        if (!window.confirm(`Delete phrase “${label}” (${id})?`)) return;
        setBusy(true);
        const { error } = await deletePhraseAction(id);
        setBusy(false);
